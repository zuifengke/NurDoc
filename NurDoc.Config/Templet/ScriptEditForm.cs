...
namespace Heren.NurDoc.Config.Templet
{
    internal partial class ScriptEditForm : DockContentBase, IScriptEditForm
    {
        ...
        public ScriptEditForm(MainForm mainForm)
            : base(mainForm)
        {
        ...
        public bool Open(DocTypeInfo docTypeInfo)
        {
        ...
        /// <returns>是否成功</returns>
        public bool Open(DocTypeInfo docTypeInfo, string szHndfFile)
        {
            ...
            {
                string szDefaultHndfFile = string.Format("{0}\\Templets\\Default.hndt"
        ...
        public void FindTextInAllTemplet(string szFindText, bool bMatchCase)
        {
            ...
            List<DocTypeData> lstDocTypeData = new List<DocTypeData>();
            NurDoc.Data.TempletService.Instance.GetFormTemplet(ref lstDocTypeData);
            ...
            for (int index = 0; index < lstDocTypeData.Count; index++)
            {
                if (lstDocTypeData[index].IsFolder)
                    continue;
                if (lstDocTypeData[index].ByteTempletData == null)
                    continue;

                parser = new FormFileParser();
                string szScripData = parser.GetScriptData(lstDocTypeData[index].ByteTempletData);
