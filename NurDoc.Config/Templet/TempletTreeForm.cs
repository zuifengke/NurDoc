...
namespace Heren.NurDoc.Config.Templet
{
    ...
    {
        public TempletTreeForm(MainForm mainForm)
            : base(mainForm)
        {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            string[] szApplyEnvs = ServerData.DocTypeApplyEnv.GetApplyEnvNames();
        ...
        private DocTypeInfo MakeDocTypeInfo(bool bIsDir)
        {
            ...
            docTypeInfo.ModifyTime = SysTimeService.Instance.Now;
        ...
        /// <returns>节点</returns>
        private TreeNode CreateNewNode(bool bIsDir, DocTypeInfo docTypeInfo)
        {
        ...
        private bool CreateFolder()
        {
            ...
            DocTypeInfo docTypeInfo = templetInfoForm.DocTypeInfo;
            if (docTypeInfo == null)
                return false;
            short shRet = DocTypeService.Instance.SaveDocTypeInfo(docTypeInfo);
            if (shRet != SystemConst.ReturnValue.OK)
            {
        ...
        private bool CreateDocType()
        {
            ...
            List<WardDocType> lstWardDocTypes = templetInfoForm.WardDocTypes;
            shRet = DocTypeService.Instance.SaveWardDocTypes(szDocTypeID, lstWardDocTypes);
            if (shRet != SystemConst.ReturnValue.OK)
            {
        ...
        private void ShowTempletInfoEditForm()
        {
            ...
            templetInfoForm.DocTypeInfo = docTypeInfo.Clone() as DocTypeInfo;
            ...
            if (docTypeInfo == null)
                return;
            short shRet = DocTypeService.Instance.ModifyDocTypeInfo(szDocTypeID, docTypeInfo);
            if (shRet != SystemConst.ReturnValue.OK)
            {
                MessageBoxEx.ShowError("模板更新失败,无法更新到数据库!");
                return;
            }
        ...
        private void DeleteNode(TreeNode deletedNode)
        {
            ...
            short shRet = DocTypeService.Instance.DeleteDocTypeInfos(lstDocTypeID);
            if (shRet != SystemConst.ReturnValue.OK)
            {
        ...
        private void MoveNode(TreeNode moveNode, bool bIsMoveToDown)
        {
            ...
            DocTypeInfo docTypeInfo = moveNode.Tag as DocTypeInfo;
            if (docTypeInfo == null)
                return;
            ...
            for (int childIndex = 0; childIndex < nodes.Count; childIndex++)
            {
                ...
                if (childNode == null)
                    continue;
                docTypeInfo = childNode.Tag as DocTypeInfo;
                if (docTypeInfo == null)
                    continue;
                int nOldValue = docTypeInfo.DocTypeNo;
                docTypeInfo.DocTypeNo = childIndex;
                short shRet = DocTypeService.Instance.ModifyDocTypeInfo(docTypeInfo.DocTypeID, docTypeInfo);
                if (shRet != SystemConst.ReturnValue.OK)
                {
                    //失败后还原被更新的对象属性
        ...
        private void DragNode(TreeNode moveNode, TreeNode targetNode)
        {
            ...
            for (int childIndex = 0; childIndex < nodes.Count; childIndex++)
            {
                ...
                if (childNode == null)
                    continue;
                DocTypeInfo docTypeInfo = childNode.Tag as DocTypeInfo;
                if (docTypeInfo == null)
                    continue;
        ...
        public void ExportTemplet()
        {
            ...
            for (int index = 0; index <= lstDocTypeInfos.Count - 1; index++)
            {
                ...
                shRet = TempletService.Instance.GetFormTemplet(docTypeInfo.DocTypeID, ref byteTempletData);
                if (shRet != SystemConst.ReturnValue.OK)
                {
        ...
        public void ImportTemplet()
        {
            ...
            for (int index = 0; index <= lstDocTypeInfos.Count - 1; index++)
            {
                ...
                //保存模板到服务器
                shRet = TempletService.Instance.SaveFormTemplet(docTypeInfo.DocTypeID, byteTempletData);
                if (shRet != SystemConst.ReturnValue.OK)
                {
                    sbExecuteResult.AppendLine("----导入失败! DocTypeName=" + docTypeInfo.DocTypeName);
                    continue;
        ...
        private void treeView1_NodeMouseDoubleClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            ...
            if (docTypeInfo != null && !docTypeInfo.IsFolder)
                TempletHandler.Instance.OpenTemplet(e.Node.Tag as DocTypeInfo);
            GlobalMethods.UI.SetCursor(this.treeView1, Cursors.Default);
        }
        ...
        private void QueryTempInfos(string sztext)
        {
            ...
            foreach (KeyValuePair<string, TreeNode> pair in nodeTable)
            {
                ...
                if (string.IsNullOrEmpty(docTypeInfo.ParentID))
                {
                    if (docTypeInfo.IsFolder)
                        this.treeView1.Nodes.Add(pair.Value);
                    else if (docTypeInfo.DocTypeName.ToLower().Contains(sztext.ToLower()))
                        this.treeView1.Nodes.Add(pair.Value);
                }
                else
                {
            ...
            for (int index = treeView1.Nodes.Count - 1; index >= 0; index--)
            {
                if (this.treeView1.Nodes.Count <= 0)
                    continue;
                DocTypeInfo DocTypeInfo = this.treeView1.Nodes[index].Tag as DocTypeInfo;
                if (this.treeView1.Nodes[index].Nodes.Count <= 0
                    && !(DocTypeInfo.DocTypeName.ToLower().Contains(sztext.ToLower())
                    && !DocTypeInfo.IsFolder))
                {
        ...
        private void CleanEmptyNode(TreeNode FatherNode, string sztext)
        {
            for (int index = FatherNode.Nodes.Count - 1; index >= 0; index--)
            {
                ...
                DocTypeInfo DocTypeInfo = FatherNode.Nodes[index].Tag as DocTypeInfo;
